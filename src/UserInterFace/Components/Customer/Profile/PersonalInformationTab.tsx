...
import { Star, Lock, LogOut } from "lucide-react"; // استدعاء الأيقونات الجديدة
...
export function PersonalInformationTab({ profileData, handleLogOut, image }: { profileData: CustomerProfileDto; handleLogOut: () => void, image: File | null }) {
  ...
  return (
    <div className="space-y-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="bg-white p-6 rounded-3xl border border-shamelco-dark/10 shadow-sm">
   
        <div className="flex items-center justify-between mb-6">
          <h3 className="font-bold text-shamelco-darker text-lg">البيانات الأساسية</h3>
          ...
          <div className="bg-status-warning/10 text-status-warning px-3 py-1.5 rounded-lg text-sm font-bold flex items-center gap-1.5 border border-status-warning/20">
            ...
            <span>{profileData?.loyaltyPoints || 0} نقطة ولاء</span>
          </div>
        </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 bg-white text-status-danger font-bold py-4 rounded-3xl border-2 border-status-danger/10 hover:bg-status-danger/5 hover:border-status-danger/30 transition-all shadow-sm active:scale-95"
      >
